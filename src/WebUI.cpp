...
static String htmlIndex() {
    ...
    h += F(
        ...
        "<button onclick=\"shipReadyReset()\" id='b_ship_ready' style='border-color:var(--warn);color:var(--warn)'>Reset Wi-Fi</button>"
...
"</table></div>"
        ...
        "<div class='card'><h2 id='t_wg'>WireGuard</h2><table>"
        ...
        "<div class='card'><h2 id='t_rtsp_urls'>RTSP URLs</h2><table>"
        "<tr><td class='k' id='t_lan_url'>LAN URL</td><td class='v'><div style='display:flex;gap:6px;align-items:center'><span class='mono' id='rtsp_lan_url'></span><button onclick=\"copyUrl('rtsp_lan_url','btn_copy_lan')\" id='btn_copy_lan' title='Copy LAN URL' style='background:none;border:1px solid var(--border);padding:2px 6px;cursor:pointer;border-radius:6px;line-height:1;font-size:12px'>Copy</button></div></td></tr>"
        "<tr><td class='k' id='t_wg_url'>WireGuard URL</td><td class='v'><div style='display:flex;gap:6px;align-items:center'><span class='mono' id='rtsp_wg_url'></span><button onclick=\"copyUrl('rtsp_wg_url','btn_copy_wg')\" id='btn_copy_wg' title='Copy WireGuard URL' style='background:none;border:1px solid var(--border);padding:2px 6px;cursor:pointer;border-radius:6px;line-height:1;font-size:12px'>Copy</button></div></td></tr>"
        "</table>"
        "<div class='hint muted' style='margin-top:8px' id='t_rtsp_url_note'>Use the WireGuard URL in BirdNet-Go when the tunnel is active.</div>"
        "</div>"
...
"function copyUrl(srcId,btnId){const el=$(srcId);if(!el)return;const t=el.textContent;const b=$(btnId);if(!b)return;const orig=b.textContent;const ta=document.createElement('textarea');ta.value=t;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();try{document.execCommand('copy');b.textContent='Copied';setTimeout(()=>{b.textContent=orig},1500)}catch(e){}document.body.removeChild(ta)}"
        ...
        "function defaultsNow(){const L=T[lang]; if(confirm(L.confirm_defaults||'Reset all settings to defaults? WireGuard configuration will also be cleared. Only the Wi-Fi connection is preserved. This cannot be undone.')){rebootSequence('factory_reset'); act('factory_reset'); }}"
...
"function loadWg(){fetch('/api/wg_status',{cache:'no-store'}).then(r=>r.json()).then(j=>{$('wg_state').innerHTML=(j.state==='up')?'<span class=ok>UP</span>':'<span class=bad>'+(j.state==='connecting'?'CONNECTING':j.state.toUpperCase())+'</span>';$('wg_last_hs').textContent=j.last_handshake;$('wg_rx').textContent=j.rx_pretty+' ('+j.rx_bytes+' bytes)';$('wg_tx').textContent=j.tx_pretty+' ('+j.tx_bytes+' bytes)';$('rtsp_lan_url').textContent=j.lan_url;$('rtsp_wg_url').textContent=j.wg_url||'N/A';const now=Date.now();const we=$('sel_wg_enable');if(we){const editing=(edits['wg_enable']&&now<edits['wg_enable']);if(!(locks['wg_enable']&&now<locks['wg_enable'])&&!editing)we.value=j.enabled?'on':'off';toggleDirty(we,'wg_enable');}const ka=$('in_wg_keepalive');if(ka){const editing=(edits['wg_keepalive']&&now<edits['wg_keepalive']);if(!(locks['wg_keepalive']&&now<locks['wg_keepalive'])&&!editing)ka.value=j.keepalive;toggleDirty(ka,'wg_keepalive');}const ep=$('in_wg_endpoint');if(ep&&!edits['wg_endpoint'])ep.value=j.endpoint;const ta=$('in_wg_tunaddr');if(ta){const editing=(edits['wg_tunaddr']&&now<edits['wg_tunaddr']);if(!(locks['wg_tunaddr']&&now<locks['wg_tunaddr'])&&!editing)ta.value=j.tunnel_addr||'';toggleDirty(ta,'wg_tunaddr');}const sp=$('in_wg_srvpub');if(sp){const editing=(edits['wg_srvpub']&&now<edits['wg_srvpub']);if(!(locks['wg_srvpub']&&now<locks['wg_srvpub'])&&!editing)sp.value=j.server_public_key;toggleDirty(sp,'wg_srvpub');}}).catch(()=>{})}"
...
"trackEdit($('sel_led'),'led_mode'); trackEdit($('in_auto'),'auto_recovery'); trackEdit($('in_thr_mode'),'thr_mode'); trackEdit($('in_sched'),'sched_reset'); trackEdit($('sel_buf'),'buffer'); trackEdit($('sel_tx'),'wifi_tx'); trackEdit($('sel_hp'),'hp_enable'); trackEdit($('sel_agc'),'agc_enable'); trackEdit($('sel_cpu'),'cpu_freq'); trackEdit($('sel_oh_enable'),'oh_enable'); trackEdit($('sel_oh_limit'),'oh_limit'); trackEdit($('sel_wg_enable'),'wg_enable'); trackEdit($('in_wg_keepalive'),'wg_keepalive'); trackEdit($('in_wg_srvpub'),'wg_srvpub'); trackEdit($('in_wg_tunaddr'),'wg_tunaddr');"
        "const H=(hid,rid)=>{const h=$(hid), r=$(rid); if(h&&r){ h.onclick=()=>{ r.style.display = (r.style.display==='none'||!r.style.display)?'block':'none'; }; }};"
"H('h_led','row_led_hint'); H('h_rate','row_rate_hint'); H('h_gain','row_gain_hint'); H('h_hpf','row_hpf_hint'); H('h_hpf_cut','row_hpf_cut_hint'); H('h_agc','row_agc_hint'); H('h_buf','row_buf_hint'); H('h_auto','row_auto_hint'); H('h_thr','row_thr_hint'); H('h_thr_mode','row_thrmode_hint'); H('h_chk','row_chk_hint'); H('h_sched','row_sched_hint'); H('h_hours','row_hours_hint'); H('h_tx','row_tx_hint'); H('h_shift','row_shift_hint'); H('h_cpu','row_cpu_hint'); H('h_level','row_level_hint'); H('h_therm_protect','row_therm_hint_protect'); H('h_therm_limit','row_therm_hint_limit'); H('h_wg_enable','row_wg_enable_hint'); H('h_wg_priv','row_wg_priv_hint'); H('h_wg_srvpub','row_wg_srvpub_hint'); H('h_wg_endpoint','row_wg_endpoint_hint'); H('h_wg_tunaddr','row_wg_tunaddr_hint'); H('h_wg_keepalive','row_wg_keepalive_hint');"
        "loadAll();"
        "</script></body></html>");
    return h;
}
