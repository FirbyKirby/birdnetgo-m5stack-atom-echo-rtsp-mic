...
"<div id='confirm_overlay' class='overlay' style='display:none'><div class='box'><p id='confirm_msg' style='margin:0 0 12px'></p><div style='display:flex;gap:8px;justify-content:center'><button id='confirm_cancel'>Cancel</button><button id='confirm_ok' class='warning'>Confirm</button></div></div></div>"
    ...
    h += F(
        ...
        "<tr><td class='k'><span id='t_hostname'>Hostname</span><span class='help' id='h_hostname'>?</span></td><td class='v'><div class='field'><input id='in_hostname' type='text' size='20' maxlength='63' placeholder='atomecho'><button id='btn_hostname_set' onclick=\"setv('hostname',in_hostname.value)\">Set</button></div></td></tr>"
...
"let confirmCallback=null;function showConfirm(msg,cb){$('confirm_msg').textContent=msg;confirmCallback=cb;$('confirm_overlay').style.display='flex'}$('confirm_cancel').onclick=function(){$('confirm_overlay').style.display='none';confirmCallback=null};$('confirm_ok').onclick=function(){$('confirm_overlay').style.display='none';if(confirmCallback)confirmCallback();confirmCallback=null};"
...
"function copyUrl(srcId,btnId){const el=$(srcId);if(!el)return;const t=el.textContent;const b=$(btnId);if(!b)return;const orig=b.innerHTML;const ta=document.createElement('textarea');ta.value=t;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();try{document.execCommand('copy');b.innerHTML='<svg width=14 height=14 viewBox=\"0 0 16 16\" fill=\"none\" stroke=\"var(--acc2)\" stroke-width=\"1.5\"><path d=\"M3 9l3 3 7-7\"/></svg>';setTimeout(()=>{b.innerHTML=orig},1500)}catch(e){}document.body.removeChild(ta)}"
        ...
        "function defaultsNow(){const L=T[lang];showConfirm(L.confirm_defaults||'Reset all settings to factory defaults?',function(){rebootSequence('factory_reset');act('factory_reset')})}"
...
"function shipReadyReset(){const L=T[lang];showConfirm(L.confirm_ship_ready||'Reset Wi-Fi settings and reboot?',function(){rebootSequence('reboot');act('ship_ready_reset')})}"
        ...
        "function normalizeJsHostname(s){if(!s)return '';s=String(s).toLowerCase();let o='';for(let i=0;i<s.length;i++){const c=s.charCodeAt(i);if((c>=97&&c<=122)||(c>=48&&c<=57)||c===45)o+=s[i];}o=o.replace(/-+/g,'-').replace(/^-+/,'').replace(/-+$/,'');if(o.length>63)o=o.slice(0,63).replace(/-+$/,'');return o;}"
        "function updateHostnamePreview(){const inp=$('in_hostname'),pv=$('hostname_preview');if(!inp||!pv)return;const n=normalizeJsHostname(inp.value);pv.textContent=(n||'atomecho')+'.local';}"
        "const locks={}; const edits={};"
        "function setv(k,v){v=String(v?\?'').trim().replace(',', '.'); if(v==='')return; if(k==='rate'){const n=parseFloat(v); if(isNaN(n))return; v=Math.round(n*1000).toString();} locks[k]=Date.now()+5000; delete edits[k]; fetch('/api/set?key='+encodeURIComponent(k)+'&value='+encodeURIComponent(v),{cache:'no-store'}).then(r=>r.json()).then(j=>{if(j.reboot){rebootSequence('reboot')}else{loadAll()}})})"
...
"trackEdit($('sel_led'),'led_mode'); trackEdit($('in_auto'),'auto_recovery'); trackEdit($('in_thr_mode'),'thr_mode'); trackEdit($('in_sched'),'sched_reset'); trackEdit($('sel_buf'),'buffer'); trackEdit($('sel_tx'),'wifi_tx'); trackEdit($('sel_hp'),'hp_enable'); trackEdit($('sel_agc'),'agc_enable'); trackEdit($('sel_cpu'),'cpu_freq'); trackEdit($('sel_oh_enable'),'oh_enable'); trackEdit($('sel_oh_limit'),'oh_limit'); trackEdit($('sel_wg_enable'),'wg_enable'); trackEdit($('in_wg_keepalive'),'wg_keepalive'); trackEdit($('in_wg_srvpub'),'wg_srvpub'); trackEdit($('in_wg_tunaddr'),'wg_tunaddr'); trackEdit($('in_wg_endpoint'),'wg_endpoint'); trackEdit($('in_wg_priv'),'wg_priv'); trackEdit($('in_hostname'),'hostname');"
        "const H=(hid,rid)=>{const h=$(hid), r=$(rid); if(h&&r){ h.onclick=()=>{ r.style.display = (r.style.display==='none'||!r.style.display)?'block':'none'; }; }};"
"H('h_led','row_led_hint'); H('h_rate','row_rate_hint'); H('h_gain','row_gain_hint'); H('h_hpf','row_hpf_hint'); H('h_hpf_cut','row_hpf_cut_hint'); H('h_agc','row_agc_hint'); H('h_buf','row_buf_hint'); H('h_auto','row_auto_hint'); H('h_thr','row_thr_hint'); H('h_thr_mode','row_thrmode_hint'); H('h_chk','row_chk_hint'); H('h_sched','row_sched_hint'); H('h_hours','row_hours_hint'); H('h_tx','row_tx_hint'); H('h_shift','row_shift_hint'); H('h_cpu','row_cpu_hint'); H('h_level','row_level_hint'); H('h_therm_protect','row_therm_hint_protect'); H('h_therm_limit','row_therm_hint_limit'); H('h_wg_enable','row_wg_enable_hint'); H('h_wg_priv','row_wg_priv_hint'); H('h_wg_srvpub','row_wg_srvpub_hint'); H('h_wg_endpoint','row_wg_endpoint_hint'); H('h_wg_tunaddr','row_wg_tunaddr_hint'); H('h_wg_keepalive','row_wg_keepalive_hint'); H('h_wg_import','row_wg_import_hint'); H('h_hostname','row_hostname_hint');"
        "loadAll();"
        "</script></body></html>");
    return h;
}
