...
static String htmlIndex() {
    ...
    h += F(
        ...
        "<button onclick=\"shipReadyReset()\" id='b_ship_ready' style='border-color:var(--warn);color:var(--warn)'>Ship-Ready Reset</button>"
...
"function shipReadyReset(){const L=T[lang]; if(confirm(L.confirm_ship_ready||'Clear WiFi credentials and reboot? WireGuard config will be preserved.')){rebootSequence('reboot'); act('ship_ready_reset');}}"
